...
const FrequentlyBought = ({ productId, heading }) => {
    ...
    const fetchRecommendations = async () => {
      ...
      try {
        const response = await fetch(
          `${SummaryApi.FrequentlyBoughtTogether.url}/${productId}`
        );
        if (!response.ok)
        ...
        {loading
          ...
          : recommendations.map((product) => (
              <Link
                ...
                to={`/product/${product.recommended}`}
                className="w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] bg-white rounded-sm shadow"
                onClick={scrollTop}
              >
                <div className="bg-slate-200 h-48 p-4 flex justify-center items-center">
                  <img
                    src={product.recommended_image || "/placeholder.png"}
                    alt={product.recommended_name || "Product"}
                ...
                <div className="p-4 grid gap-3">
                  ...
                  <button
                    ...
                    onClick={(e) => handleAddToCart(e, product.recommended)}
                  >
                    Add to Cart
                  </button>
                </div>
